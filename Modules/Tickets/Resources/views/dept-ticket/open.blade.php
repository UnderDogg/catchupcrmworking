...
@section('inbox')
  ...
  if (Auth::guard('staff')->user()->role == 'agent') {
    $tickets = Modules\Core\Models\Ticket\Tickets::where('status', '=', '1')->where('isanswered', '=', 0)->where('dept_id', '=', $dept->id)->orderBy('id', 'DESC')->paginate(20);
  } else {
    $tickets = Modules\Core\Models\Ticket\Tickets::where('status', '=', '1')->where('isanswered', '=', 0)->where('dept_id', '=', $dept->id)->orderBy('id', 'DESC')->paginate(20);
  }
  ?>
  ...
  <div class="box box-primary">
    <div class="box-header with-border">
      <h3 class="box-title">{!! Lang::get('tickets::lang.open') !!} </h3>
      <small id="title_refresh">{!! $tickets->total() !!} {!! Lang::get('tickets::lang.tickets') !!}</small>
    ...
    @if(Session::has('success'))
      ...
      <div class="box-body">
        ...
        <div class="mailbox-controls">
          ...
          <input type="submit" class="submit btn btn-default text-orange btn-sm" id="delete" name="submit"
                 value="{!! Lang::get('tickets::lang.delete') !!}">
          <input type="submit" class="submit btn btn-default text-yellow btn-sm" id="close" name="submit"
                 value="{!! Lang::get('tickets::lang.close') !!}">
          <button type="button" class="btn btn-sm btn-default text-green" id="Edit_Ticket" data-toggle="modal"
                  data-target="#MergeTickets"><i class="fa fa-code-fork"> </i> {!! Lang::get('tickets::lang.merge') !!}</button>
        </div>
        <div class="mailbox-messages" id="refresh">
          <p style="display:none;text-align:center; position:fixed; margin-left:40%;margin-top:-70px;" id="show"
             class="text-red"><b>{!! Lang::get('tickets::lang.loading') !!}...</b></p>
          <!-- table -->
          {!! Datatable::table()
              ->addColumn(
                  "",
      ...
      <div class="modal-content">
              ...
              <div class="row">
                ...
                <div class="col-md-6">
                  ...
                  <select class="form-control" id="select-merge-parent" name='p_id'
                          data-placeholder="{!! Lang::get('tickets::lang.select_tickets') !!}" style="width: 100%;">
                    <option value=""></option>
                  </select>
                </div>
              </div>
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-default pull-left" data-dismiss="modal"
                  id="dismis2">{!! Lang::get('tickets::lang.close') !!}</button>
          <input type="submit" id="merge-btn" class="btn btn-primary pull-right"
                 value="{!! Lang::get('tickets::lang.merge') !!}"></input>
          {!! Form::close() !!}
        </div><!-- /.modal-footer -->
      </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
    ...
    <div class="modal-dialog" role="document">
      ...
      <div class="col-md-8">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-primary pull-left yes"
                    data-dismiss="modal">{{Lang::get('tickets::lang.ok')}}</button>
