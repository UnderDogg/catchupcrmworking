...
if (Auth::guard('staff')->user()->role == 'agent') {
    $dept = App\Model\helpdesk\Agent\Department::where('id', '=', Auth::guard('staff')->user()->primary_dpt)->first();
    $tickets = App\Model\helpdesk\Ticket\Tickets::where('status', '=', 1)->where('assigned_to', '=', Auth::guard('staff')->user()->id)->orderBy('id', 'ASC')->paginate(20);
} else {
    $tickets = App\Model\helpdesk\Ticket\Tickets::where('status', '=', 1)->where('assigned_to', '=', Auth::guard('staff')->user()->id)->orderBy('id', 'ASC')->paginate(20);
}
?>
